...
Leave blank to skip keyword scanning.
    ...
    # Fresh run — reset everything and kick off
    if run or dry:
    ...
    # Process next batch — auto-reruns until complete
    active_keywords = st.session_state.run_keywords
    sample          = st.session_state.total_sample
    start_idx       = st.session_state.next_index

    if st.session_state.is_running and sample and start_idx < len(sample):
        batch      = sample[start_idx : start_idx + BATCH_SIZE]
        overall_pct = start_idx / len(sample)
        progress   = st.progress(overall_pct, text=f"Running... {start_idx}/{len(sample)} done")
        status_box = st.empty()
        ...
        for i, person in enumerate(batch):
            ...
            progress.progress((global_i+1)/len(sample), text=f"{global_i+1}/{len(sample)} complete")
            time.sleep(0.1)

        st.session_state.next_index = start_idx + len(batch)
        status_box.empty(); progress.empty()

        # Auto-advance: if not done, immediately rerun to process next batch
        if st.session_state.next_index < len(sample):
            st.rerun()
        else:
